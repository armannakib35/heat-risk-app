...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .results-container { background: rgba(255,255,255,0.95); border-radius: 24px; padding: 16px; }
    ...
    .route-stats { display: flex; gap: 16px; font-size: 13px; margin-top: 8px; }
...
<div id="heatMapContent" class="tab-content active">
  <div class="app-container">
    ...
    <div class="map-container"><h3>🗺️ Heat Risk Map</h3><div id="heatMap"></div></div>
    ...
    <div class="control-panel"><h4 style="margin-bottom:12px;">🤖 AI Agent Network</h4><div class="agents-grid" id="agentsGrid"></div></div>
    <div class="results-container"><h3>📊 Nearby Street Heat Analysis</h3><div id="resultsContent">Waiting for location data...</div></div>
  </div>
</div>

<div id="routeContent" class="tab-content">
  <div class="app-container">
    <div class="route-header"><h2>🗺️ Smart Route Planner</h2><p style="color:#666; margin-top:8px;">Find the coolest path to your destination</p></div>
    <div id="routeMap"></div>
    <div class="search-section">
      <div class="search-box">
        <input type="text" class="search-input" id="destinationInput" placeholder="Enter destination (e.g., Central Park)">
    ...
    <div class="route-results"><h3>🎯 Route Options</h3><div id="routeResultsContent">Enter a destination to see route options</div></div>
  </div>
</div>
...
function switchTab(tab) {
  ...
  if (tab === 'heatMap') {
    ...
    if (heatMap) setTimeout(() => heatMap.invalidateSize(), 100);
  } else {
...
function initHeatMap(lat, lon) {
  ...
  else userMarker = L.marker([lat, lon]).addTo(heatMap);
...
async function refreshHeatMap() {
  ...
  navigator.geolocation.getCurrentPosition(async function(position) {
    ...
    document.getElementById('resultsContent').innerHTML = '<div class="loading">✅ Data refreshed successfully</div>';
  }, function(error) {
    document.getElementById('resultsContent').innerHTML = '<div class="loading">❌ Location access denied</div>';
  });
}
...
function planRoutes() {
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    const isBest = route.isBest || false;
  ...
  html += '<div class="route-card" style="background:#e8f4f8; margin-top:10px;"><div class="route-name">💡 Heat Safety Tips</div>' +
    '<div class="route-stats">• Walk on shaded side of street</div><div class="route-stats">• Carry water and umbrella</div>' +
