  ...
  <div id="heatmapPanel">
    ...
    <div id="warningCard" class="warning-card safe" onclick="refreshHeatmap()">
      ...
      <div class="warning-message" id="warningMessage">
        Tap to check your current heat risk level. Stay hydrated and protected from sun exposure.
      </div>
    ...
    <div class="control-panel">
      <div class="slider-container">
        <div class="slider-label">
          ...
          <span id="forecastValueHeat">Now</span>
        </div>
        <input type="range" id="forecastSliderHeat" min="0" max="12" value="0" oninput="updateHeatForecast(this.value)">
      </div>
      <button class="search-btn" style="width:100%;" onclick="refreshHeatmap()">
        🔄 Refresh Data
      </button>
    </div>
...
function switchTab(tab) {
  ...
  if (tab === 'heatmap') {
    ...
    if (!routeMap && currentLat) {
      ...
      routeMap.setView([currentLat, currentLon], 14);
    }
  }
}
...
function initHeatmapMap(lat, lon) {
  if (!heatmapMap) {
    heatmapMap = L.map("heatmap").setView([lat, lon], 15);
  ...
  }).addTo(heatmapMap).bindPopup('<b>You are here</b>');
}

function updateHeatForecast(value) {
  currentForecast = parseInt(value);
  document.getElementById('forecastValueHeat').innerText = forecastNames[currentForecast];
...
function initRouteMap(lat, lon) {
  if (!routeMap) {
    routeMap = L.map("routemap").setView([lat, lon], 14);
    L.tileLayer("https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png", {
      attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>',
      subdomains: 'abcd',
      maxZoom: 19
  ...
  window.userMarkerRoute = L.marker([lat, lon], {
    icon: L.divIcon({
      html: '<div style="background-color:#ff7e5f; width:20px; height:20px; border-radius:50%; border:3px solid white;"></div>',
      iconSize: [20, 20]
    })
  }).addTo(routeMap).bindPopup('<b>Start: Your Location</b>');
}
...
async function snapToNearestRoad(lat, lon) {
  const query = `[out:json][timeout:5];way["highway"](around:100,${lat},${lon});out center;`;
  try {
    const response = await fetch("https://overpass-api.de/api/interpreter", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": "AI-Heat-Risk-Demo/1.0" },
    ...
    const data = await response.json();
    if (data.elements && data.elements.length > 0 && data.elements[0].center) {
